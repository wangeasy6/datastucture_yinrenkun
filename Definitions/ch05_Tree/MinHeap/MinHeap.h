...
// 构造函数：建立空堆
template </*class T*/typename Item>MinHeap</*T,*/ Item>::MinHeap(int sz){
...
// 构造函数：通过一个数组建堆
template </*class T*/typename Item> MinHeap</*T,*/ Item>::MinHeap(Item arr[], int n)
{
...
//私有函数：最小堆的下滑调整算法
template </*class T*/typename Item>void MinHeap</*T,*/ Item>::siftDown(int start, int m){
...
// 私有函数：最小堆的上滑调整算法
template </*class T*/typename Item>void MinHeap</*T,*/ Item>::siftUp(int start){
	int j = start, i = (j-1)/2;
	Item temp = heap[j];
...
// 公共函数: 将x插入到最小堆中
template </*class T*/typename Item>bool MinHeap</*T,*/ Item>::Insert(const Item &x){
...
// 公共函数：最小堆的删除算法
template </*class T*/typename Item>bool MinHeap</*T,*/ Item>::RemoveMin(Item &x){
